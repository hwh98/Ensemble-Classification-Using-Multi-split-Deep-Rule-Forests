# simply replicate the as.party.C5.0() function to avoid the incorrect of rule extraction.
validate.as.party.C5.0 <- function(obj, trial = 0, ...) {
  # some text preparation from C50() model$output
  out <- strsplit(obj$output, "\n")[[1]]
  out <- out[out != ""]
  out <- out[grep("^\t", out, invert = TRUE)]
  out <- out[grep("\\*\\*\\*", out, invert = TRUE)]
  tr <- as.vector(obj$trials) # whether the object is decicion tree
  # if (tr > 1) {
  #   if (trial > (tr - 1)) {
  #     trial = tr - 1
  #   }
  ...
  #   out <- out[(iv1 + 2):(iv2 - 1)]
  # } else{
    # extract the rule only content 
    indx <- 1:which(out == "Decision tree:") # the line number where the rule start
    out <- out[-indx] # remove unrelated content 
    l1 = length(out) 
    out <- out[1:(l1 - 2)] #remove the last two line
  #}
    
  
  check1 <- cbind(grep("\\{", out), grep("\\}", out)) # line number where it contain categorical variable
  indv1 <- which(check1[, 1] != check1[, 2]) # line where categorical variable in more than one line 
  # combine those multi-line factor variable into one line 
  if (length(indv1) > 0) {
    a17 <- check1[indv1, , drop = FALSE] # get indv1 
    rml = NULL
    for (j in 1:dim(a17)[1]) {
      nterms = diff(a17[j, ]) # how many line of that multi-line category variable 
      vlaps = NULL
      for (i in 1:nterms) {
  ...
  # C50() object whether contain "Term" (a info about C50 training )
  is.default <- !("Terms" %in% names(obj))
  if (!is.default) { # run this part 
    mf <- model.frame(obj) # get the training data 
  } else{
  ...
  # only have a root node
  if (length(out) == 1) { 
    pn <- as.partynode(partynode(1L), from = 1L)
  } else{ 
    # run this
    n.cat <-sapply(1:length(obj$pred), function(i)is.factor(mf[, obj$pred[i]]))# whether predictor is factor -> TRUE FALSE FALSE TRUE
    adj.pred<-as.vector(sapply(obj$pred,function(i){gsub("`","",i)}))
    # iterate each line of rule to clean 
    f.mat <- lapply(1:length(out), function(i) { 
      # remove the predictor from the rule 
      # "...Loan_Amount <= 2900" " Charged Off (95/47)"  
      valpred<-integer(0)
      vec<-strsplit(out[i],":")[[1]] # get the if-else and category part 
      vec<-vec[vec!=""]
      varp<-as.vector(sapply(adj.pred,function(j){ # it mark that predictor as 1 other as -1
        # adj.pred -> "Loan_Grade"    "Loan_Amount"   "Interest_Rate" "Loan_Purpose" 
        # vec : "...Loan_Amount <= 2900" " Charged Off (95/47)"  
        # varp = -1 1 -1 -1 
      ...
      # a1 remove the predictor part from the if-else statement in rule  
      # a1 : ":... <= 2900: Charged Off (95/47)"
      
      if(n.cat[valpred]){ # if the predictor is factor(true) # n.cat -> TRUE FALSE FALSE TRUE 
        ##process this 
        if(length(grep(" in \\{",a1))>0){ # whether a1 contain "in {" pattern
          vec<-a1
          while(length(grep("^in",vec))==0){ # remove . at the start of the line
            vec<-sub("^.","",vec)
          } # become  in {B,D,E}: Charged Off (69/32)
          a2<-sub("in \\{","",vec) # become B,D,E}: Charged Off (69/32)
          if(length(grep(":",a2))>0){
            a2<-strsplit(a2,"\\}:") # "B,D,E"                " Charged Off (69/32)"
            if(length(a2)>2){
          ...
          a2<-a2[[1]][1] # B,D,E
          a1<-sub(a2,"X",vec) # become in {X}: Charged Off (69/32)
          a2<-paste0("{",a2,"}",collapse="") # {B,D,E}
        }else{ # skip else
          vec<-a1
      ...
      a1 <- strsplit(a1, " ")[[1]] # "in"      "{X}:"    "Charged" "Off"     "(69/32)"
      a1 <- gsub(":", "", a1) # "in"      "{X}"     "Charged" "Off"     "(69/32)"
      a1 <- gsub("\\.\\.\\.", "", a1)
      a1 <- a1[a1 != ""]
      if(n.cat[valpred]){ # whether this line is factor
        a1[2]<-a2 #"in"      "{B,D,E}" "Charged" "Off"     "(69/32)"
      }
      as.vector(c(adj.pred[valpred],a1)) #  "Loan_Grade" "in"         "{B,D,E}"    "Charged"    "Off"     "(69/32)" 
    })
    # f.mat[[14]] become "Loan_Grade" "in"         "{B,D,E}"    "Charged"    "Off"   "(69/32)" 
    
    # iterarte through f.mat(rule)
    indvars <- sapply(1:length(f.mat), function(i) { # return the predictor index of each rule 
      v = match(obj$predictors, f.mat[[i]][1]) # the predictor of each line index in obj$pred
      a1 <- which(!is.na(v))
      if (length(a1) == 0)
        a1 = 0
      a1
    ...
    treestr <- sapply(1:length(out), function(i) {
      ...
      avec = nchar(strsplit(out[[i]], avec)[[1]][1]) # count the char before the rule start to know which level it is 
      avec
    })# 0 4 4 0 4 4 8 8 12 12 16 16 16 20 20 .....
    treestr <- as.numeric(as.factor(treestr)) # turn to numeric
    # 1 2 2 1 2 2 3....
    ...
    } # list contain 2 factor predictor 'Loan_Grade' 'Loan_Purpose' and its levels
    
    # function used in C5.node
    c5.split <-
    ...
    # function 
    # tvec - treestr ; vvec - indvars ; bvec - cuts ; vvars - vars  
    c5.node <- function(tvec, vvec, bvec, vvars) {
      ...
      for (i in 1:lind) {
        str = ind[i]
        if (i == lind) {
          term = length(tvec) # the length of treestr
        } else{
          term = ind[i + 1] - 1
        }
        val = ind[i]:term # subtract the rule-1 
  ...
  if (is.default) { # False skip if 
    ...
    # run this party (is.defualt = FALSE)
    p1 <- all.vars(attr(obj$Terms, "predvars"))[attr(obj$Terms, "response")] # get the target variable `loan_status`
    if (is.na(p1)) {
